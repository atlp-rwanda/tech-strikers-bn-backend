...
const sendNotification = async (target, urlTarget, subject, content) => {
  // const url = `${process.env.APP_URL}/requests/${tripRequest.id}`;
  const url = `${process.env.APP_URL}/${urlTarget}`;
  await transport.sendMail({
    from: `${process.env.EMAIL_SENDER}`,
    to: `${target.fullname} <${target.email}>`,
    subject,
    html: `
    <div>
      <p>Greetings ${target.fullname},</p>
      <p>${content}</p>
    </div>`,
  });
};
