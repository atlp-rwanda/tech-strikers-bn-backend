...
export default class TripControllers {
  ...
  /**
   * @description this controller get a request by id in database
   * @param {object} req request
   * @param {object} res response
   * @return {object} returns json object with signup message
   */

  static async getOneWayTripRequestById(req, res) {
    const trip = await getRequestById(req.params.id);

    return getResponseOnGet(res, trip);
  }
}
